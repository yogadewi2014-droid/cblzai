...
function setupTelegramHandler(bot) {
    ...
    // 📢 FITUR SUARA (GO/PRO) - eksplisit
    bot.command('suarakan', async (ctx) => {
        const userId = `telegram:${ctx.from.id}`;
        const text = ctx.message.text.replace('/suarakan', '').trim();
        if (!text) return ctx.reply('Tulis teks yang ingin disuarakan. Contoh: /suarakan Halo, apa kabar?');

        if (await getUserTier(userId) === 'free') return ctx.reply('Fitur ini hanya untuk pengguna GO dan PRO.');

        const quotaCheck = await checkMediaQuota(userId, 'ffmpeg');
        if (!quotaCheck.allowed) return ctx.reply(quotaCheck.reason || 'Kuota harian sudah habis.');

        try {
            const { generateVoice } = require('../services/voiceOutput');
            const lang = detectLanguage(text);
            const voice = selectVoice(lang);
            const audioBuffer = await generateVoice(text, lang, voice);
            const tmpFile = require('path').join(require('os').tmpdir(), `yenni-tts-${Date.now()}.mp3`);
            fs.writeFileSync(tmpFile, audioBuffer);
            await ctx.replyWithAudio({ source: tmpFile });
            fs.unlinkSync(tmpFile);
            await consumeQuota(userId, 'ffmpeg');
        } catch (error) {
            logger.error('Voice error:', error);
            ctx.reply('Maaf, ada gangguan saat membuat suara.');
        }
    });

    // 🎬 FITUR VIDEO (GO/PRO) - eksplisit
    bot.command('video', async (ctx) => {
        const userId = `telegram:${ctx.from.id}`;
        const text = ctx.message.text.replace('/video', '').trim();
        if (!text) return ctx.reply('Tulis teks untuk video. Contoh: /video Fotosintesis pada tumbuhan.');
        ...
        try {
            await ctx.reply('🎬 Yenni sedang membuat video... Mohon tunggu sebentar ya~');
            ...
            const videoPath = await buildMedia(text, imageUrl);
            await ctx.replyWithVideo({ source: videoPath });
            if (videoPath && fs.existsSync(videoPath)) fs.unlinkSync(videoPath);
            await consumeQuota(userId, 'ffmpeg');
        } catch (error) {
            logger.error('Video error:', error);
            ctx.reply('Maaf, ada gangguan saat membuat video. Coba lagi ya.');
        }
    });
    ...
    bot.on('text', async (ctx) => {
        ...
        try {
            ...
            if (result.voicePath && fs.existsSync(result.voicePath)) {
                await ctx.replyWithAudio({ source: result.voicePath });
